...
int main(void){
	...
	std::uniform_real_distribution<long double> UnifffTail(1.02L, 2.98L);            //Select \mu range 
	...
	long double PowerTail = UnifffTail(generatorDos);            //Use randomly generated \mu
	...
	std::string FileName;
	if(PowerTail > 3.0L){
		FileName = "DestructiveBrownianMot_" + RandomString(16);
	}else{
		FileName = "DestructiveLevyFlights_" + RandomString(16);
	}
	
	fout.open(FileName.c_str());
	
	std::cout << "Destructive Forager " << FileName.c_str() << std::endl;
	std::cout << PowerTail << " tail \\mu" << std::endl;
	std::cout << MaxSight << " Sight Radius r_v" << std::endl;
	fout << PowerTail << " tail \\mu" << std::endl;
	...
	long double MeanFreePathLambda = 1.0L/(2.0L*MaxSight*(nrolls/(2.0L * MapBounds * 2.0L * MapBounds)));
	
	fout << MeanFreePathLambda << " Mean Free Path \\lambda" << std::endl;
	

    
    std::cout << "Initializing Nodes with density " << nrolls/(2.0L * MapBounds * 2.0L * MapBounds) << std::endl;
	std::cout << MeanFreePathLambda << " Mean Free Path \\lambda" << std::endl;
	...
	long double totalDispl = 0.0L;
		...
		do{ //Scoped
			...
			do{
				...
				auto FirstEncounter = findEncounteredNode(NodePositions, PositionForager, AFlight, MaxSight, &Foundflag);
				if(Foundflag != (size_t)-1){
					...
					std::cout << "Node: " << NodePositions[Foundflag].x_axis << " " << NodePositions[Foundflag].y_axis << std::endl;  //This contains the point at which travel line intersects with MaxSight influence circle
					auto DirectDist = calculateDistance(NodePositions[Foundflag], PositionForager);
					...
					PositionForager = NodePositions[Foundflag];
					NodePositions.erase(NodePositions.begin() + Foundflag);
					itter++;   //Success
					Foundflag = -1;	
					
					if((itter%5) == 0){  //Scoped, save results every some successes
						fout << std::endl;
						fout << std::endl;
	...
	fout  << std::endl;
	fout << PowerTail << " tail \\mu" << std::endl;
	fout << MaxSight << " Sight Radius r_v" << std::endl;
	...
	fout << AverageFlightCount << " flights N" << std::endl;
	fout << AverageSuccessLength << " Distance Units per success" << std::endl;
  	fout << AverageFlightLength << " Distance Units per flight <l>" << std::endl;
  	fout << MeanFreePathLambda << " Mean Free Path \\lambda" << std::endl;
